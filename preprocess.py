...
datasets = ['synthetic', 'SMD', 'SWaT', 'SWaT_ori','SMAP', 'MSL', 'WADI', 'MSDS', 'UCR', 'MBA', 'NAB']
...
def convertNumpy_swat(df):
	x = df.values[::10, :]
	result = (x - x.min(0)) / (x.ptp(0) + 1e-4)
	return result
...
def convertNumpys(dff):
	x = dff[dff.columns[1:]].values[::10, :]
...
def fft_apply(data_ori):
	...
	for i in range(data.shape[0]):
		fft_x = data[i]
		fft_x = fft(np.array(fft_x))  
		fft_x = np.fft.ifft(fft_x)
		data[i] = fft_x
	data = data.swapaxes(0, 1)
	plt.plot(data)
	plt.show()
...
def load_data(dataset):
	...
	if dataset == 'synthetic':
		...
		print("***",df_train.shape,df_test.shape,labels.shape)
		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file).astype('float64'))
	elif dataset == 'SWaT':
		dataset_folder = 'data/SWaT/'
		df_train = pd.read_csv(dataset_folder + "SWaT_Dataset_Normal_v1.csv")
		df_train = df_train.drop(['Timestamp', 'Normal0/Attack1'], axis=1)
		df_test = pd.read_csv(dataset_folder + "SWaT_Dataset_Attack_v0.csv")
		labels = [float(label != 0) for label in df_test['Normal0/Attack1'].values]
		print("sum_label", np.sum(labels))
		df_test = df_test.drop(['Timestamp', 'Normal0/Attack1'], axis=1)
		print("swat_df_train.shape", df_train.shape)
		print("swat_df_test.shape", df_test.shape)
		...
		train, test, labels = convertNumpy_swat(pd.DataFrame(train)), convertNumpy_swat(
			pd.DataFrame(test)), convertNumpy_swat(pd.DataFrame(labels))

		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file))
	elif dataset == 'SWaT_ori':
		dataset_folder = 'data/SWaT_ori/'
		df_train = pd.read_csv(dataset_folder + "SWaT_Dataset_Normal_v1.csv")
		...
		size = train.shape[0]
		labels = np.broadcast_to(np.array(labels).reshape(-1, 1), test.shape)
		train, test, labels = convertNumpy_swat(pd.DataFrame(train)), convertNumpy_swat(pd.DataFrame(test)), convertNumpy_swat(pd.DataFrame(labels))
		print(train.shape, test.shape, labels.shape)
		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file))
	elif dataset == 'SWaT_full':
		dataset_folder = 'data/SWaT_full'

		train = pd.read_csv(os.path.join(dataset_folder, 'swat_train.csv'), skiprows=1000)
		test_all = pd.read_csv(os.path.join(dataset_folder, 'swat_test.csv'))
		sensor_list = (np.array(
			[1, 2, 6, 7, 8, 9, 17, 18, 19, 26, 27, 28, 29, 35, 36, 37, 38, 39, 40, 41, 42, 45, 46, 47,
			 48]) - 1).tolist()
		train = train.iloc[:, 0:52]
		test = test_all.iloc[:, 0:52]


		labels = pd.read_csv("swat_label.csv")
		print("labels", labels.shape)
		train, test = convertNumpys(train), convertNumpys(test)
		labels = convertNumpys(labels)
		print(train.shape)
		print(test.shape)
		print(labels.shape)
		train = train[:, sensor_list]
		test = test[:, sensor_list]
		labels = labels[:, sensor_list]
		print(train.shape, test.shape, labels.shape)
		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file))
		...
		train = not_all_empty_col_in_dataframe(train)
		test = not_all_empty_col_in_dataframe(test)
		print("train", train)
		print("test", test)
		...
		for file in ['train', 'test', 'labels']:
			np.save(os.path.join(folder, f'{file}.npy'), eval(file))
	elif dataset == 'WADI_FFT':
		dataset_folder = 'data/WADI_FFT'
		ls = pd.read_csv(os.path.join(dataset_folder, 'WADI_attacklabels.csv'))
		train = pd.read_csv(os.path.join(dataset_folder, 'WADI_14days.csv'), skiprows=1000, nrows=2e5)
		test = pd.read_csv(os.path.join(dataset_folder, 'WADI_attackdata.csv'))
		train.dropna(how='all', inplace=True); test.dropna(how='all', inplace=True)
